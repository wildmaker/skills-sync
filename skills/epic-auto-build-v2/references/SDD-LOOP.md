...
3. 使用 `@tool-use-openspec-init-change` 初始化 OpenSpec 变更（inputs: `<spec-name>`, `<branch>`；`<spec-name>` 同时作为 spec name 与 change ID，对应目录 `OpenSpec/changes/<spec-name>`）
...
运行 `harness-agent-review-loop`（inputs: `<base-branch>`, `<issue>`, `<spec-name>`；其中 `<base-branch>` 应为 `epic/<epic-name>`；在 PR 中引用对应的 Issue 和 OpenSpec 变更）。
...
8. **通过单独 PR 归档该 OpenSpec 变更：**
   ...
   - 提交归档变更后，**同样必须**运行 `harness-agent-review-loop` 等待并处理 High/Medium 评论，再合并归档 PR（不要“提交→秒合并”）
