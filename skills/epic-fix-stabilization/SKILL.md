...
- `skills/epic-auto-build-v2/references/epic-workflow.md`（分支/合并约束）
...
4. 运行 `harness-agent-review-loop` 创建 PR 并处理评审闭环：`<fix-branch>` → `<epic-branch>`（base 必须是 `<epic-branch>`；PR 中引用 `<fix_issues>`；不需要 Spec Change），并确保：
