...
10. 通过单独 PR 归档该 OpenSpec 变更（同样必须走评审闭环；不要“提交→秒合并”错过自动 reviewer）：
   ...
   - 提交归档变更
   - 运行 `agent-review-loop` 创建 PR（base 必须是 `<epic-branch>`；PR 中引用同一个 Issue 与 `<spec-name>`）
   - 按 `agent-review-loop` 完成 High/Medium 评论闭环后，再用 `git-merge-recent-pr` 合并（默认不删除 head 分支）
