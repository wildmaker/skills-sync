---
name: agent-review-loop-resolve-pr-comments
description: 提供 agent-review-loop-resolve-pr-comments 的操作流程与约束；适用于 需要拉取 PR 评论并处理高/中优先级意见时使用。
version: 0.2.0
---
...
3. 跨 reviewer 去重归并：
   - 将所有 reviewer（local-cli / bugbot / gemini 等）的评论按 `file + line range` 分组。
   - 同一区域的多条意见合并为一条待办，优先级取最高（High > Medium > Low），修复建议取最具体的一条。
   - 去重后输出统一待办清单，标注每条意见的来源 reviewer（便于回复时 @对应 reviewer）。
4. 汇总并分流评论优先级（High/Medium/Low），并只对 High/Medium 做强制闭环：
   - High/Medium：必须给出“已修复”或“无法修复的明确理由 + 替代方案/后续动作”
   - Low：可选处理；不处理也需在汇总里标注为 Low（避免漏看）
5. 处理 High/Medium 评论：
   - 按去重后的统一清单逐条落实到代码改动或文档/说明
   - 若一次修复同时解决了多个 reviewer 的意见，在各 reviewer 对应评论下均做回复说明
   - 对应的回复策略：简短说明“如何修/为何不修/如何验证”
6. 若产生代码改动：直接执行 `git add`、`git commit`、`git push` 推送到 PR 原分支。
   - 提交信息建议：`fix(review): address PR comments`（或包含 backlog/spec id 以便追踪）
   - 尽量一次 push 完成所有修复，避免多次 push 触发多余的 re-review 循环
7. 复查（最多 2 轮）：
